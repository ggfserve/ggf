export const _siteUrl =
  process.env.NEXT_PUBLIC_URL_BASE || 'http://localhost:3047';

export const _mail = 'Admin@ggfservices.com';
export const _phone = '(780) 437-1011';
export const _address = '15397 117 Ave, # 204';
export const _addressLink =
  'https://www.google.com/maps/place/Go+Green+Facilities+Services+%26+Janitorial/@53.4653411,-113.4825403,15z/data=!4m6!3m5!1s0x2ab98b72a4d7da87:0x30b64c0885a3f145!8m2!3d53.4653411!4d-113.4825403!16s%2Fg%2F11y4643dqq?entry=ttu';
export const _googleLink = 'https://maps.app.goo.gl/XyWQgWHjMVw1kRbn7';
export const _facebook = 'https://www.facebook.com/ggfservices/';
export const _instagram = 'https://www.instagram.com/ggfservices/';

export const _linkedin =
  'https://www.linkedin.com/company/go-green-facilities-services/';

export const _services = [
  {
    title: 'Cleaning',
    description:
      'Discover eco-friendly cleaning services by Go Green, offering sustainable solutions for commercial spaces to ensure a clean, healthy work environment.',
    link: '/services/cleaning',
    linkTitle: 'Cleaning Services'
  },
  {
    title: 'Integrated Facility',
    description:
      'Go Green`s Integrated Facility Services combine expert cleaning, maintenance, and management for efficient upkeep of commercial properties.',
    link: '/services/integrated-facility',
    linkTitle: 'Integrated Facility Services'
  },
  {
    title: 'Specialized Services',
    description:
      'Explore Go Green`s specialized cleaning services, tailored to meet unique commercial needs with environmentally conscious practices and advanced technology.',
    link: '/services/specialized',
    linkTitle: 'Specialized Services'
  },

  {
    title: 'Disinfection & Sanitization',
    description:
      'Ensure a germ-free environment with Go Green`s disinfection and sanitary processing services, using eco-safe products for effective cleaning.',
    link: '/services/sanitization',
    linkTitle: 'Disinfection & Sanitization Services'
  },
  {
    title: 'Environmental',
    description:
      'Go Green`s Environmental Services promote sustainability in commercial cleaning, reducing your carbon footprint while maintaining pristine facilities.',
    link: '/services/environmental',
    linkTitle: 'Environmental Services'
  },
  {
    title: 'Post-Construction Cleaning',
    description:
      'Post-construction cleanup by Go Green offers thorough, eco-friendly cleaning services to prepare new spaces for safe and immediate use.',
    link: '/services/post-construction',
    linkTitle: 'Post-Construction Cleaning Services'
  },
  {
    title: 'Commercial Floor Cleaning',
    description:
      'Professional commercial floor cleaning services by Go Green use green methods to maintain and restore the beauty of your floors sustainably.',
    link: '/services/commercial-floor-cleaning',
    linkTitle: 'Commercial Floor Cleaning Services'
  },
  {
    title: 'Outdoor Building Maintenance',
    description:
      'Maintain your building`s exterior with Go Green`s Outdoor Building Maintenance Services, ensuring a clean and professional appearance year-round.',
    link: '/services/outdoor-building-maintenance',
    linkTitle: 'Outdoor Building Maintenance Services'
  }, {
    title: 'Carpet Steam Cleaning',
    description:
      'Deep carpet steam cleaning services that eliminate stains, allergens, and dirt using eco-friendly technology for healthier indoor spaces.',
    link: '/services/carpet-steam-cleaning',
    linkTitle: 'Carpet Steam Cleaning Services'
  },
  // {
  //   title: 'Residential Cleaning',
  //   description:
  //     'Trusted residential cleaning solutions tailored to your schedule, ensuring a spotless home using safe and effective green products.',
  //   link: '/services/residential-cleaning',
  //   linkTitle: 'Residential Cleaning Services'
  // },
  {
    title: 'Janitorial Cleaning',
    description:
      'Comprehensive janitorial services for all facility types, keeping workspaces clean, hygienic, and well-maintained with minimal disruption.',
    link: '/services/janitorial-cleaning',
    linkTitle: 'Janitorial Cleaning Services'
  },
  {
    title: 'Window Cleaning',
    description:
      'Professional window cleaning that leaves glass spotless and streak-free, enhancing natural light and curb appeal for any property.',
    link: '/services/window-cleaning',
    linkTitle: 'Window Cleaning Services'
  },
  {
    title: 'Junk Removal',
    description:
      'Fast and responsible junk removal services for residential and commercial properties, including furniture, debris, and more.',
    link: '/services/junk-removal',
    linkTitle: 'Junk Removal Services'
  },
  {
    title: 'Building Pressure Washing',
    description:
      'Restore the look of your building with high-quality pressure washing that removes dirt, mold, and grime from all surfaces.',
    link: '/services/building-pressure-washing',
    linkTitle: 'Building Pressure Washing Services'
  },
  {
    title: 'Garage & Warehouse Cleaning',
    description:
      'Heavy-duty cleaning for garages and warehouses, ensuring safety, cleanliness, and operational efficiency with industrial-grade equipment.',
    link: '/services/garage-warehouse-cleaning',
    linkTitle: 'Garage & Warehouse Cleaning Services'
  },
  {
    title: 'Underground Parkade & Lot Cleaning',
    description:
      'Thorough cleaning services for underground parkades and lots, including sweeping and power washing for a safer, cleaner facility.',
    link: '/services/parkade-lot-cleaning',
    linkTitle: 'Underground Parkade & Lot Cleaning Services'
  },
  {
    title: 'Driveway Pressure Wash & Sealing',
    description:
      'Enhance and protect your driveway with expert pressure washing and sealing to prevent damage and improve curb appeal.',
    link: '/services/driveway-pressure-washing-sealing',
    linkTitle: 'Driveway Pressure Wash & Sealing Services'
  }

];

export const _industries = [
  {
    title: 'Commercial',
    description:
      'Go Green provides comprehensive, green cleaning solutions for commercial spaces, enhancing your business environment and customer experience.',
    link: '/industries/commercial',
    linkTitle: 'Commercial Services',
    img: '/images/industries/commercial/banner.png'
  },

  {
    title: 'Educational',
    description:
      'Safe and sustainable cleaning services for educational institutions by Go Green, creating healthy learning environments for students and staff.',
    link: '/industries/educational',
    linkTitle: 'Educational Services',
    img: '/images/industries/educational/banner.png'
  },
  {
    title: 'Industrial',
    description:
      "Go Green's industrial cleaning services ensure clean, compliant, and efficient operations with environmentally responsible cleaning practices.",
    link: '/industries/industrial',
    linkTitle: 'Industrial Services',
    img: '/images/industries/industrial/banner.png'
  },
  {
    title: 'Institutional',
    description:
      "Go Green's institutional cleaning services provide safe and sustainable solutions for government and public facilities, ensuring clean and healthy environments.",
    link: '/industries/institutional',
    linkTitle: 'Institutional Services',
    img: '/images/industries/institutional/main.png'
  },
  {
    title: 'Hospitality',
    description:
      "Enhance guest experiences with Go Green's hospitality cleaning services, offering sustainable solutions for hotels, restaurants, and entertainment venues.",
    link: '/industries/hospitality',
    linkTitle: 'Hospitality Services',
    img: '/images/industries/hospitality/banner.png'
  },
  {
    title: 'Healthcare',
    description:
      "Go Green's healthcare cleaning services prioritize safety and hygiene, using non-toxic products to maintain clean and healthy medical facilities.",
    link: '/industries/healthcare',
    linkTitle: 'Healthcare Services',
    img: '/images/industries/healthcare/banner.png'
  },
  {
    title: 'Recreational',
    description:
      "Keep recreational facilities inviting and clean with Go Green's eco-conscious cleaning services, from gyms to public parks and community centers.",
    link: '/industries/recreational',
    linkTitle: 'Recreational Services',
    img: '/images/industries/recreational/banner.png'
  }
];

// lib/data/blogPosts.ts

export interface BlogPost {
  slug: string;
  title: string;
  excerpt: string;
  content: string;
  publishedAt: string;
  readingTime?: string;
  category?: string;
}

export const blogPosts: BlogPost[] = [
  {
    slug: 'eco-cleaning-benefits',
    title: 'Top 5 Benefits of Eco-Friendly Cleaning for Your Business',
    excerpt: 'Discover how green cleaning methods improve workplace health, sustainability, and employee productivity while protecting the environment.',
    publishedAt: '2024-12-10',
    readingTime: '5 min read',
    category: 'Sustainability',
    content: `
      Eco-friendly cleaning has become a rising trend as businesses seek to reduce their environmental impact while maintaining high cleanliness standards. Here are the top 5 benefits of adopting green cleaning practices:

      1. Improved Indoor Air Quality
      Traditional cleaning products often contain harsh chemicals that can compromise indoor air quality. Green cleaning solutions use natural ingredients that don't release harmful VOCs, creating a healthier workplace environment.

      2. Enhanced Employee Wellness
      By eliminating exposure to harsh chemicals, eco-friendly cleaning practices help reduce allergic reactions, respiratory issues, and skin irritations among employees. This leads to fewer sick days and increased productivity.

      3. Environmental Impact
      Green cleaning products are biodegradable and packaged in recyclable containers, significantly reducing your business's environmental footprint. These products are made from renewable resources and don't contribute to water or air pollution.

      4. Cost-Effective Solutions
      While some green products may have a higher upfront cost, they often require less product to achieve the same results. Additionally, concentrated formulas and reduced packaging waste lead to long-term cost savings.

      5. Brand Enhancement
      Implementing eco-friendly cleaning practices demonstrates your commitment to sustainability, which can improve your company's image and appeal to environmentally conscious customers and employees.
    `
  },
  {
    slug: 'janitorial-vs-commercial-cleaning',
    title: 'Janitorial vs Commercial Cleaning: What’s the Difference?',
    excerpt: 'Learn the key differences between janitorial services and commercial cleaning to choose the best option for your facility needs.',
    publishedAt: '2024-12-02',
    readingTime: '4 min read',
    category: 'Services',
    content: `
      Many people confuse janitorial services with commercial cleaning, but there are distinct differences between these two types of professional cleaning services.

      1. Frequency and Scope
      Janitorial services typically involve daily or regular maintenance tasks like emptying trash, vacuuming, and maintaining restrooms. Commercial cleaning, however, focuses on deeper, more specialized cleaning projects performed less frequently.

      2. Skill Level and Training
      Janitorial staff are trained in day-to-day cleaning and maintenance tasks. Commercial cleaners often require specialized training and equipment for specific services like floor stripping, carpet extraction, or high-rise window cleaning.

      3. Equipment and Resources
      Janitorial services usually use standard cleaning equipment and supplies. Commercial cleaning services invest in specialized equipment and industrial-grade cleaning solutions for more intensive cleaning tasks.

      4. Cost Structure
      Janitorial services are typically contracted on a regular basis with monthly or annual agreements. Commercial cleaning services are often project-based or scheduled periodically for specific deep-cleaning needs.

      5. Time of Service
      Janitorial services usually occur during off-hours to maintain daily cleanliness. Commercial cleaning projects may require specific scheduling and can sometimes interrupt regular business operations.

      Choosing between janitorial and commercial cleaning depends on your facility's specific needs, budget, and cleaning requirements. Many businesses benefit from combining both services for optimal facility maintenance.
    `
  },
  {
    slug: 'pressure-washing-tips',
    title: 'When and Why You Should Invest in Pressure Washing',
    excerpt: 'Understand how pressure washing restores exterior surfaces, prevents damage, and boosts curb appeal for your property.',
    publishedAt: '2024-11-25',
    readingTime: '3 min read',
    category: 'Maintenance',
    content: `
      Pressure washing isn't just about improving appearance. It's a crucial maintenance task that can protect your property's value and prevent costly repairs.

      1. Optimal Timing
      Spring and fall are ideal seasons for pressure washing, removing winter grime or summer buildup. Regular cleaning prevents permanent staining and surface degradation.

      2. Property Protection
      Regular pressure washing removes harmful substances like mold, mildew, and algae that can damage building materials over time. This preventive maintenance extends the life of your exterior surfaces.

      3. Cost Benefits
      While pressure washing requires an initial investment, it prevents expensive repairs and replacements by maintaining your property's surfaces. It's much more cost-effective than replacing damaged materials.

      4. Health Considerations
      Pressure washing eliminates allergens, mold, and other harmful substances that can affect air quality and health. This is particularly important for commercial properties with high foot traffic.

      5. Professional Results
      While DIY pressure washing is possible, professional services ensure the right pressure levels and cleaning solutions for different surfaces, preventing damage while achieving optimal results.
    `
  }
];
